...
describe('openVenmoPayment', () => {
  ...
  it('opens valid Venmo payment link', () => {
    mockWindowOpen.mockReturnValue({} as Window); // Mock successful window open
...
describe('shareVenmoPayment', () => {
  ...
  // Test when navigator.share is not available
...
describe('formatVenmoNote', () => {
  it('formats note with restaurant and person name', () => {
    const note = formatVenmoNote('Pizza Palace', 'Alice');
    expect(note).toBe('Pizza Palace - Alice');
  });

  it('formats note with only restaurant name', () => {
  ...
  it('truncates long notes to Venmo limit', () => {
    const longRestaurant = 'A'.repeat(VENMO_CONFIG.MAX_NOTE_LENGTH);
    const note = formatVenmoNote(longRestaurant, 'Alice');

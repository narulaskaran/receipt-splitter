...
/**
 * Formats a note for Venmo payment based on restaurant name and person name
 * 
...
export function formatVenmoNote(
  ...
  if (trimmedRestaurant && trimmedPerson) {
    note = `${trimmedRestaurant} - ${trimmedPerson}`;
  } else if (trimmedRestaurant) {
    note = trimmedRestaurant;

...
export enum SplitDataError {
  ...
  RESTAURANT_NAME_TOO_LONG = 'RESTAURANT_NAME_TOO_LONG',
...
export function serializeSplitData(
  ...
  const validation = validateSerializationInput(people, restaurant, date, phone);
...
export function validateSplitDataDetailed(splitData: SharedSplitData): SplitValidationResult {
  ...
  try {
    ...
    if (splitData.phone && !isValidPhoneNumber(splitData.phone)) {
    ...
    if (splitData.restaurant && splitData.restaurant.length > VALIDATION_LIMITS.MAX_RESTAURANT_NAME_LENGTH) {
      errors.push(SplitDataError.RESTAURANT_NAME_TOO_LONG);
      errorMessages.push(`Restaurant name exceeds ${VALIDATION_LIMITS.MAX_RESTAURANT_NAME_LENGTH} characters`);
    }
...
export function validateSerializationInput(
  ...
  if (restaurant && restaurant.length > VALIDATION_LIMITS.MAX_RESTAURANT_NAME_LENGTH) {
    errors.push(SplitDataError.RESTAURANT_NAME_TOO_LONG);
    errorMessages.push(`Restaurant name exceeds ${VALIDATION_LIMITS.MAX_RESTAURANT_NAME_LENGTH} characters`);
  }
